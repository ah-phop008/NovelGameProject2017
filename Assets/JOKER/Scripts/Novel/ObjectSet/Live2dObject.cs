...
namespace Novel{
	...
	public class Live2dObject : AbstractObject {
		...
		public override void setPosition(float x,float y,float z){
			// RenderTexture分ポジションをずらす
			float posY = this.rootObject.GetComponent<FadeInOut> ().transY;
			this.rootObject.transform.position = new Vector3(x,y + posY,z);
		}
		...
		public override void show(float time,string easeType){
            this.rootObject.GetComponent<FadeInOut> ().FadeIn (time);
//			this.rootObject.GetComponent<SimpleModel>().enabled = true;
		...
		public override void hide(float time,string easeType){
			...
			this.rootObject.GetComponent<FadeInOut> ().FadeOut (time);
